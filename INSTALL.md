...
The hook only fires when the flag file exists, so installing the plugin changes nothing by itself. "stop adhd mode" still turns it off for the current session, and it stays off: a companion `UserPromptSubmit` hook records it, so the re-injection that fires on resume and compaction leaves it alone. Invoke `/i-have-adhd` again to turn it back on.

That same hook restates in one line, on each prompt, that the ruleset applies — long sessions wear an injected ruleset down, and the line re-anchors it.
